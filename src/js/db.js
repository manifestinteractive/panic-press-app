	...
	init: function()
	{
		...
		else
		{
			...
			M.migration(1, function(tx){
				...
				tx.executeSql("CREATE TABLE `panic_history` ( `id` integer NOT NULL PRIMARY KEY AUTOINCREMENT, `short_url` text(50,0), `status` text(25,0) NOT NULL DEFAULT 'sending', `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, `last_modified` text )");
